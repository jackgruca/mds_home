...
class TradeService {
  ...
  double _adjustForTeamNeeds(double probability, String teamName, int pickNumber) {
    final teamNeed = teamNeeds.firstWhere(
      ...
      orElse: () => null as TeamNeed, // Will throw error if not found
    );
...
class TradeInterest {
  ...
  const TradeInterest({
    ...
    if (competitorsExistForPosition(player.position, targetPickNumber, teamNextPick)) {
    ...
    interest += (_random.nextDouble Function() Function  * 0.2) - 0.1; // ±0.1 randomness
  ...
  bool _competitorsExistForPosition(String position, int currentPick, int teamNextPick) {
    ...
    for (int i = currentPick + 1; i < teamNextPick; i++) {
      ...
      final competitorPick = draftOrder.firstWhere(
        (pick) => pick.pickNumber == i && !pick.isSelected,
        orElse: () => null as DraftPick, // Will throw error if not found
      );
      
      if (competitorPick == null) continue;
      
      // Check team needs
      final competitorNeeds = teamNeeds.firstWhere(
        (needs) => needs.teamName == competitorPick.teamName,
        orElse: () => null as TeamNeed, // Will throw error if not found
      );
      
      if (competitorNeeds == null) continue;
  ...
  List<TradePackage> _generateFirstRoundPackages(
    ...
    final List<TradePackage> packages = [];
    final bool isTopTenPick = targetPick.pickNumber <= 10;
    final String targetTeam = targetPick.teamName;
    
    // Strategy 1: Current pick + future pick (common for top picks)
    if (isTopTenPick && profile.futurePickAffinity >= 0.4) {
      final futurePick = FuturePick.forRound(teamName, 1); // Future 1st
      
      if (bestPick.pickNumber > targetPick.pickNumber &&
          bestPick.pickNumber <= targetPick.pickNumber + 40 &&
          DraftValueService.getValueForPick(bestPick.pickNumber) + futurePick.value >= requiredValue * 0.95) {
        packages.add(TradePackage(
          teamOffering: teamName,
          teamReceiving: targetTeam,
          picksOffered: [bestPick],
          targetPick: targetPick,
          totalValueOffered: DraftValueService.getValueForPick(bestPick.pickNumber) + futurePick.value,
    ...
    // Strategy 2: Multiple current year picks (most common)
    if (teamPicks.length >= 2) {
      // Try combinations of two picks first
      for (int i = 0; i < min(teamPicks.length, 3); i++) {
        ...
        for (int j = i + 1; j < min(teamPicks.length, i + 5); j++) {
          ...
          // If two picks aren't enough, try three picks
          else if (teamPicks.length > j + 1 && combinedValue >= requiredValue * 0.7) {
            for (int k = j + 1; k < min(teamPicks.length, j + 3); k++) {
              final thirdPick = teamPicks[k];
              final threePickValue = combinedValue + 
                                     DraftValueService.getValueForPick(thirdPick.pickNumber);
              
              if (threePickValue >= requiredValue * 0.95) {
                packages.add(TradePackage(
                  teamOffering: teamName,
                  teamReceiving: targetTeam,
  ...
  List<TradePackage> _generateSecondRoundPackages(
    ...
    if (teamPicks.length >= 2) {
      ...
      for (int i = 1; i < min(teamPicks.length, 4); i++) {
    ...
    if (profile.futurePickAffinity >= 0.3) {
      // Mid-round future pick for 2nd round trades
      const futureRound = 3;  
      final futurePick = FuturePick.forRound(teamName, futureRound);
      
      if (DraftValueService.getValueForPick(bestPick.pickNumber) + futurePick.value >= requiredValue * 0.95) {
        packages.add(TradePackage(
          teamOffering: teamName,
          teamReceiving: targetTeam,
          picksOffered: [bestPick],
    ...
    // Strategy 3: Single pick if close in value (sometimes happens in 2nd round)
    if (DraftValueService.getValueForPick(bestPick.pickNumber) >= requiredValue * 0.98) {
      packages.add(TradePackage(
        teamOffering: teamName,
        teamReceiving: targetTeam,
        picksOffered: [bestPick],
...
return _random.nextDouble() < acceptanceProbability;
  ...
  double _adjustForTeamNeeds(double probability, String teamName, int pickNumber) {
    ...
    if (teamNeed == null) return probability;
