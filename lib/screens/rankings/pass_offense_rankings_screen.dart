...
class PassOffenseRankingsScreen extends StatefulWidget {
  const PassOffenseRankingsScreen({super.key});

  @override
  State<PassOffenseRankingsScreen> createState() => _PassOffenseRankingsScreenState();
}

class _PassOffenseRankingsScreenState extends State<PassOffenseRankingsScreen> {
  ...
  void _updateStatFields() {
    if (_showRanks) {
      ...
      _statFields = {
        'totalYds': {'name': 'Pass Yards', 'format': 'integer', 'description': 'Total passing yards'},
        'totalTD': {'name': 'Pass TDs', 'format': 'integer', 'description': 'Total passing touchdowns'},
        'successRate': {'name': 'Success Rate', 'format': 'percentage', 'description': 'Passing success rate'},
  ...
  Future<void> _loadRankings() async {
    ...
    try {
      ...
      final statFields = _statFields.keys.where((key) => 
        !['myRankNum', 'posteam', 'tier', 'season', 'passOffenseTier'].contains(key)
      ).toList();
      ...
      setState(() {
        _error = 'Failed to load pass offense rankings: ${e.toString()}';
        _isLoading = false;
      });
    }
  }
  ...
  Widget _buildDataTable() {
    if (_rankings.isEmpty) {
      return const Center(
        child: Text(
          'No pass offense rankings found for the selected criteria.',
  ...
  List<DataColumn> _buildDataColumns() {
    final columns = <DataColumn>[
      ...
      DataColumn(
        ...
        onSort: (columnIndex, ascending) => _sort('passOffenseTier', ascending),
      ),
    ];
  ...
  List<DataRow> _buildDataRows() {
    return _rankings.asMap().entries.map((entry) {
      ...
      final tierValue = team['passOffenseTier'] ?? team['tier'] ?? 1;
