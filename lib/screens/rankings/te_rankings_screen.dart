...
class TERankingsScreen extends StatefulWidget {
  ...
  @override
  State<TERankingsScreen> createState() => _TERankingsScreenState();
}
...
class _TERankingsScreenState extends State<TERankingsScreen> {
  ...
  void initState() {
    ...
    // Update SEO meta tags for TE Rankings page
  ...
  Future<void> _loadTERankings() async {
    ...
    try {
      ...
      setState(() {
        _error = 'Failed to load TE rankings: ${e.toString()}';
        _isLoading = false;
      });
    }
  }
  ...
  Widget _buildDataTable() {
    if (_teRankings.isEmpty) {
      return const Center(
        child: Text(
          'No TE rankings found for the selected criteria.',
  ...
  List<DataRow> _buildDataRows() {
    return _teRankings.asMap().entries.map((entry) {
      ...
      if (_selectedSeason == 'All Seasons') {
        cells.add(DataCell(Text(te['season']?.toString() ?? '')));
      }
