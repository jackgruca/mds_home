...
class _CustomizeDraftTabViewState extends State<CustomizeDraftTabView>
  ...
  void initState() {
    ...
    Future<void> loadLastSession() async {
      ...
      final autoSave = allDataSets.firstWhere(
        (data) =>
            data.name == 'Auto Save ${widget.selectedYear}' &&
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      if (_teamNeeds == null) {
        final teamNeeds =
            await DataService.loadTeamNeeds(year: widget.selectedYear);
      ...
      if (_playerRankings == null) {
        final players =
            await DataService.loadAvailablePlayers(year: widget.selectedYear);
    ...
    return Column(
      children: [
        ...
        Container(
          ...
          child: Row(
            ...
            children: [
              ...
              Consumer<AuthProvider>(
                builder: (context, authProvider, _) => !authProvider.isLoggedIn
                    ? GestureDetector(
                        onTap: () {
                          showDialog(
                            context: context,
                            builder: (context) =>
                                const AuthDialog(initialMode: AuthMode.signIn),
                          );
                        },
                        child: const Row(
                          children: [
        ...
        TabBar(
          ...
          indicatorColor:
              Theme.of(context).primaryColor, // Keep the indicator color
        ),
        Expanded(
          child: TabBarView(
            controller: _tabController,
