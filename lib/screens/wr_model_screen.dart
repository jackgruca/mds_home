...
class _WRModelScreenState extends State<WRModelScreen> {
  ...
  List<Map<String, dynamic>>?
      _wrCacheAll; // No longer used for full caching, but keeping for now if needed for future viz
  ...
  final List<QueryOperator> _allowedOperators = [
    QueryOperator.equals
  ]; // Will remove this if allowing all operators
    ...
    for (int i = 0; i < _pagesToPreload; i++) {
      ...
      try {
        final HttpsCallable callable =
            functions.httpsCallable('getWrModelStats');
        final result = await callable.call<Map<String, dynamic>>({
          'filters': filtersForFunction,
          'limit': _rowsPerPage,
          'orderBy': _sortColumn,
                            ...
                            if (fieldType == 'height') {
                                ...
                                children: [
                                  Flexible(
                                    child: TextField(
                                      ...
                                      decoration: const InputDecoration(
                                          labelText: 'ft', isDense: true),
                                      onChanged: (val) {
                                        _newQueryValueController.text =
                                            val.isEmpty ? '0' : val;
                                      },
                                    ),
                                  ),
                              ...
                              return InkWell(
                                onTap: () async {
                                  ...
                                  if (picked != null) {
                                    _newQueryValueController.text =
                                        '${picked.month.toString().padLeft(2, '0')}/${picked.day.toString().padLeft(2, '0')}/${(picked.year % 100).toString().padLeft(2, '0')}';
                                    setState(() {});
                                  }
                                },
                                child: IgnorePointer(
  ...
  Widget _buildDataTable() {
    if (_rawRows.isEmpty && !_isLoading && _error == null) {
      return const Center(
          child: Text(
              'No WR data found. Try adjusting your filters or check the database.',
      ...
      if (selectedGroupIndex == 0) {
        ...
        for (final row in _rawRows) {
          ...
          if (!playerMap.containsKey(id) ||
              (row['season'] ?? 0) > (playerMap[id]?['season'] ?? 0)) {
            playerMap[id] = row;
          }
        }
                      ...
                      if (selected) {
                        setState(() {
                          ...
                          if (index != fieldGroups.length - 1) {
                            _selectedFields =
                                List<String>.from(fieldGroups[index]['fields']);
                          }
                        });
                      }
                    },
            ...
            child: SingleChildScrollView(
              child: SingleChildScrollView(
                ...
                padding: const EdgeInsets.all(
                    8.0), // Add padding around the DataTable
                child: Theme(
                  ...
                  child: DataTable(
                    headingRowColor: WidgetStateProperty.resolveWith<Color?>(
                        (Set<WidgetState> states) => headerColor),
                    ...
                    columns: displayFields.map((header) {
                      return DataColumn(
                            ...
                            children: [
                              ...
                              if (_sortColumn == header)
                                Icon(
                                  ...
                                  size: 16,
                                  color: Colors
                                      .white, // Sort icon color for header
                                ),
                            ],
                          ),
                        ),
                        ...
                        tooltip:
                            'Sort by ${headerDisplayNames[header] ?? header}',
                      );
                    }).toList(),
                        ...
                        cells: displayFields.map((header) {
                          ...
                          if (value != null) {
                            if (value is num &&
                              ...
                              if (percentile != null) {
                                ...
                                if (percentile > 0.85) {
                                  cellStyle = cellTextStyle.copyWith(
                                      fontWeight: FontWeight.bold);
                                }
                              }

                              // Smart numeric formatting
                              if (header.toLowerCase().contains('share') ||
                                  header.toLowerCase().contains('pct') ||
                                  header.toLowerCase().contains('%')) {
                                displayValue =
                                    '${(value * 100).toStringAsFixed(1)}%';
                              } else if (value is double) {
                                // Specific formatting for different metrics
                                if (header.toLowerCase() == 'adot' ||
                                    header.toLowerCase() == 'yprr' ||
                                    header.toLowerCase().contains('epa')) {
                          ...
                          if (value != null) {
                            ...
                            if (header == 'birth_date') {
                              ...
                              if (date != null) {
                                displayValue =
                                    '${date.month.toString().padLeft(2, '0')}/${date.day.toString().padLeft(2, '0')}/${(date.year % 100).toString().padLeft(2, '0')}';
                              } else {
                                displayValue = value.toString();
                              }
                            }
              ...
              child: Row(
                ...
                children: [
                  ...
                  Builder(builder: (context) {
                    ...
                    debugPrint(
                        'Previous Button: _currentPage = \$_currentPage, onPressed = ${canGoPrevious ? 'enabled' : 'disabled'}');
                  ...
                  Builder(builder: (context) {
                    ...
                    final bool canGoNextCache = _queryConditions.isEmpty &&
                        (_currentPage + 1) * _rowsPerPage < _totalRecords;
                    final bool canGoNext = canGoNextFirebase || canGoNextCache;
                    debugPrint(
                        'Next Button: _nextCursor = \$_nextCursor, _queryConditions.isEmpty = \${_queryConditions.isEmpty}, (_currentPage + 1) * _rowsPerPage = \${(_currentPage + 1) * _rowsPerPage}, _totalRecords = \$_totalRecords, onPressed = ${canGoNext ? 'enabled' : 'disabled'}');
                    return ElevatedButton(
                          ...
                          ? () {
                              setState(() {
                                ...
                                if (_pageCursors.length <= _currentPage) {
                                  ...
                                  final cursorToAdd = _nextCursor ??
                                      _preloadedCursors[_currentPage];
                                  if (cursorToAdd != null &&
                                      cursorToAdd != 'cached_data_exists') {
                                    // Avoid adding the dummy value
                                    _pageCursors.add(cursorToAdd);
                                  } else if (_pageCursors.length >
                                      _currentPage) {
                                    // If next page is cached, and we don't have its cursor, add null placeholder
