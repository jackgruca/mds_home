...
const TE_RANKINGS_COLLECTION = 'te_rankings_comprehensive';
const INPUT_JSON_PATH = path.join(__dirname, './te_rankings.json');
...
if (!fs.existsSync(INPUT_JSON_PATH)) {
    ...
    console.error("Please run the 'te_rankings.R' script first to generate this file.");
    process.exit(1);
}
...
console.log('Total TE Rankings:', data.length);
...
async function uploadTEData() {
    console.log('=== TE RANKINGS UPLOAD ===');
    console.log(`TE Rankings: ${data.length} records`);
    console.log('');

    try {
        // Upload TE Rankings
        const teCollection = db.collection(TE_RANKINGS_COLLECTION);
        await uploadBatchedData(teCollection, data, 'TE Rankings', 'receiver_player_id', 'season');

        console.log('');
        console.log('🎉 ALL UPLOADS COMPLETE!');
        console.log('TE Rankings: Comprehensive TE metrics and tier classifications');
        console.log('Collection: te_rankings_comprehensive');
        console.log('===========================================');
