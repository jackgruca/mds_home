...
#### WR rankings ####
WR_stats <- rbind(pbp_2024, pbp_2023, pbp_2022, pbp_2021, pbp_2020, pbp_2019, pbp_2018, pbp_2017, pbp_2016) %>%
...
WR_ranks <- left_join(WR_stats, wr_tgt_share) %>% left_join(.,rz_rate) %>% left_join(.,explosive_rate) %>% left_join(.,WR_ngs) %>% left_join(.,third_down_rate) %>%
  filter(player_position == "WR") %>%
  group_by(season) %>% mutate(EPA_rank = percent_rank(avgEPA), tgt_rank = percent_rank(tgt_share), YPG_rank = percent_rank(numYards/numGames), 
                              td_rank = percent_rank(totalTD/numGames), conversion_rank = percent_rank(conversion), explosive_rank = percent_rank(explosive_rate), 
                              ...
                              third_down_rank = percent_rank(third_down_rate), first_down_rank = percent_rank(FDperTgt), yacOE_rank = percent_rank(yac_above_expected), YACperRec_rank = percent_rank(YACperRec)) %>%
...
# Export to CSV
output_path <- "../../data_processing/assets/data/wr_season_stats.csv"
# Create directory if it doesn't exist
dir.create(dirname(output_path), recursive = TRUE, showWarnings = FALSE)
write.csv(WR_ranks, output_path, row.names = FALSE)
print(paste("WR stats CSV exported to:", output_path))
print(paste("Total rows:", nrow(WR_ranks)))
print(paste("Seasons included:", paste(unique(WR_ranks$season), collapse = ", ")))